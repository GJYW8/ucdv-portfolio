  ...
  main.variable(observer("viewof order")).define("viewof order", ["d3","html","Promises"], function(d3,html,Promises)
{
  const options = [
    ...
    {name: "Order by group", value: (a, b) => a.group - b.group || d3.ascending(a.name, b.name)},
    {name: "Order by degree", value: (a, b) => d3.sum(b.sourceLinks, l => l.value) + d3.sum(b.targetLinks, l => l.value) - d3.sum(a.sourceLinks, l => l.value) - d3.sum(a.targetLinks, l => l.value) || d3.ascending(a.name, b.name)}
  ];
  ...
  main.variable(observer("chart")).define("chart", ["d3","graph","DOM","width","height","margin","y","color","arc","step","viewof order","invalidation"], function(d3,graph,DOM,width,height,margin,y,color,arc,step,$0,invalidation)
{
  console.log("graph");
  const linkValue = d3.scaleSqrt().range([1, 4]).domain(d3.extent(graph.links, d => d.value));
...
`);
  ...
  const path = svg.insert("g", "*")
    ...
    .join("path")
      ...
      .attr("stroke-width", d => linkValue(d.value))
      .attr("d", arc);
  ...
  function update() {
    y.domain(graph.nodes.sort($0.value).map(d => d.name));
    ...
    label.transition(t)
        ...
        .attrTween("transform", d => {
          const i = d3.interpolateNumber(d.y, y(d.name));
...
d3.scalePoint(graph.nodes.map(d => d.name).sort(d3.ascending), [margin.top, height - margin.bottom])
)});
  main.variable(observer("margin")).define("margin", function(){return(
{top: 20, right: 20, bottom: 20, left: 130}
)});
  ...
  main.variable(observer("graph")).define("graph", ["data"], function(data)
{
    ...
    const nodes = data.nodes.map(({name, group}) => ({
    name,
    sourceLinks: [],
    targetLinks: [],
    group
  ...
  const nodeById = new Map(nodes.map(d => [d.name, d]));
