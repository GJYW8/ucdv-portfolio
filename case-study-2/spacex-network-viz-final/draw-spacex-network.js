...
export default function define(runtime, observer) {
  ...
  main.variable(observer("drawChart")).define("drawChart", ["data", "d3", "width", "height", "color", "drag", "invalidation"], function(data, d3, width, height, color, drag, invalidation) {
    return (
      function drawChart() {
        ...
        const textFixedInitial = svg.append('g')
          .selectAll('text')
          .data(nodes)
          .join('text')
          .attr("data-target", d => d.group)
          .text( function(d) {
            ...
            var initial = "";
            initial = d.group.substring(0,1);
            initial = initial.toUpperCase();
            
            if ( d.group == "launchpad" ) {
              // avoid double L: launch / launchpad
              // use S for launch *S*ite
              initial = "S";
            }
        ...
        function fade(opacity) {
          return d => {
            ...
            textFixedInitial.style('opacity', function(o) {
              return isConnected(d, o) ? 1 : opacity
            });
  ...
  main.variable(observer("color")).define("color", ["d3"], function(d3) {
    const scale = d3.scaleOrdinal(["#FF7000", "#3569a4", "#cc0000", "#808080", "#90A324"]);
